...
void CSceneMgr::init()
{
	...
	PortalObject = pPMeshData->Instantiate();
	PortalObject->SetName(L"Portal");
	...
	PortalObject->Collider2D()->SetBB(BoundingBox(PortalObject->Transform()->GetLocalPos(), PortalObject->MeshRender()->GetMesh()->GetBoundingBoxExtents()));
	PortalObject->Collider2D()->SetBS(BoundingSphere(PortalObject->Transform()->GetLocalPos(), PortalObject->MeshRender()->GetMesh()->GetBoundingSphereRadius() / 2.f));
