...
void CBossScript::init()
{
	...
	//GetObj()->GetScript<CBossScript>()->SetColSSA(pSwordCol);
...
void CBossScript::update()
{
	...
	//CBossScript* Boss = GetObj()->GetScript<CBossScript>();
	...
	//
	//Boss->Transform()->SetLocalRot(vRot);

	//if (m_Is_Move) {
	//	AnimationPlay(Boss_ANI_TYPE::WALK);
...
void CBossScript::SetBossAnimationData(Ptr<CMesh> AniDate, const int& i, const UINT& _StartFrame, const UINT& _EndFrame)
{
...
void CBossScript::SetBossAnimation(const int& i)
{
...
void CBossScript::SetBossAnimation(int other_id, const int& i)
{
...
void CBossScript::AnimationPlay(const Boss_ANI_TYPE& type)
{
...
void CBossScript::AnimationPlay(int other_id, const Boss_ANI_TYPE& type)
{
	if (type == Boss_ANI_TYPE::IDLE)
	{
		GameObject.find(other_id)->second->Animator3D()->SetCurClip(0);
		SetBossAnimation(other_id, 0);
	}
