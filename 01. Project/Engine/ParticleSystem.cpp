...
CParticleSystem::CParticleSystem()
	...
	, m_fStartScale(2.5f)
	, m_fEndScale(10.f)
	, m_vStartColor(Vec4(0.8f, 0.8f, 0.0f, 1.4f))
	, m_vEndColor(Vec4(1.0f, 1.0f, 1.0f, 1.0f))
	, m_bType(true)
{
	// 구조화 버퍼 생성
	...
	Ptr<CTexture> pParticle = CResMgr::GetInst()->Load<CTexture>(L"Texture\\Particle\\CartoonSmoke.png", L"Texture\\Particle\\smokeparticle.png");
...
void CParticleSystem::Init(float frequency, float acctime, float minLife, float maxLife, float minSpeed, float maxSpeed, float StartScale, float EndScale, Vec4 StartColor, Vec4 EndColor)
{
